...
- **`db.Migrator.SchemaDrift(ctx, models...)`** — schema-level drift detection complementing the file-level `Migrator.Drift()`. Introspects the live database via `pragma_table_info` (SQLite) or `information_schema.columns` (Postgres, MySQL) and compares against `model.ExtractMeta` to report four new drift kinds: `schema_missing_table`, `schema_missing_column`, `schema_extra_column`, `schema_column_nullability`. MSSQL and Oracle return `db.ErrSchemaDriftUnsupported` — per-dialect introspection there is a follow-up. Closes audit `2026-05-14-post-sprint-readiness` §3 row 9 / §7 task 8.
- **Live-DB integration tests for `app.AutoMigrate`** — `pkg/app/automigrate_live_test.go::TestSQLMatrix_AutoMigrate` (Postgres + MySQL, `NUCLEUS_SQL_MATRIX_URL`) and `TestSQLMatrix_AutoMigrate_Exploratory` (MSSQL + Oracle, `NUCLEUS_SQL_EXPLORATORY_URL`) call `App.AutoMigrate` against the live container, then introspect `information_schema` to verify the table and column NOT NULL / nullable polarity match what `model.ExtractMeta` declared. Closes the gap flagged by audit `2026-05-14-post-sprint-readiness` §5 risk 4 / §7 task 7 — `db-matrix-required` previously exercised only the CLI smoke and `SELECT 1`.
