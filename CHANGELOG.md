...
- **Changed (`nucleus new` scaffolder — ADR-010 Phase 4, Slice 2): both `api` and `mvc` templates now generate projects on the fluent `pkg/nucleus` surface.** Generated `main.go` (at repo root; `go run .`) uses `nucleus.New().FromConfigFile(...).[WithoutDefaults().]WithOpenAPI(...).Mount(...).Start()`; schema is managed via a scaffolded `migrations/` directory and `nucleus migrate` (SQL-first, multi-dialect) — the hand-written `ensureSchema`/`ensureSeed` SQLite-only Go DDL helpers are removed from generated code. The composition root moves from `cmd/server/main.go` to root `main.go`. Each generated resource is a self-contained feature module under `internal/<resource>/module.go` implementing `nucleus.Module[C]`, with a repository and service wired against the framework-managed DB in `OnStart(ctx, rt nucleus.Runtime, _)` and controllers migrated to `*nucleus.Context`. Admin models register via the module's `Models` field; OpenAPI via `AppBuilder.WithOpenAPI`. The `mvc` template's blanket anonymous RBAC allow (`AddPolicy("anonymous","/api/*","*")` — all methods) is replaced by a declarative, scoped `rbac_policy.csv` (wired via `admin_rbac_policy_file`) granting anonymous read-only access on the public surface and create on the demo resource; admin write operations still require login. The `nucleus new` command name, flags, and positional arguments are unchanged — no CLI contract change. Backward compatible for existing projects (only newly scaffolded projects are affected). See [ADR-010](docs/adrs/ADR-010-fluent-api-v2-pkg-nucleus.md) Phase 4, Slice 2.
