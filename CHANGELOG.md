...
* **deps:** raise the sibling module floors to nucleus v1.29.0 ([#570](https://github.com/jcsvwinston/nucleus/issues/570)) ([8707d38](https://github.com/jcsvwinston/nucleus/commit/8707d380caa6dbef1ea9fc3151540d828c82ddba))
...
* **ldap:** re-pina el framework a v1.17.1 ([#355](https://github.com/jcsvwinston/nucleus/issues/355)) ([4fa35c3](https://github.com/jcsvwinston/nucleus/commit/4fa35c37ebf8a9a888fdbb6bbb58c31db412ea19))
...
* **release:** retira el release-as consumido del proveedor ([#337](https://github.com/jcsvwinston/nucleus/issues/337)) ([f7001e9](https://github.com/jcsvwinston/nucleus/commit/f7001e93ba888a48c9bf47167f1c32e2aacd3718))
...
* QCD-FW-5/6/7 — plantillas recursivas, knobs de outbox con owner por instancia, Flush sin pánico ([#260](https://github.com/jcsvwinston/nucleus/issues/260)) ([d44bde6](https://github.com/jcsvwinston/nucleus/commit/d44bde6ded42b4eac3f4da82c5eab0d050dfe454))
...
- **`examples/showcase_demo` — the Quantum suite showcase, restored.** A runnable Nucleus application whose domain runs on the Quark ORM with the Orbit admin mounted on top, wiring both suite integration bridges (quantum QADR-0006): `orbit/quarkbridge` feeds the live SQL view with the app's Quark statements correlated to the request, and `orbit/quarkdatasource` backs Data Studio with the same Quark models via `orbit.Config.DataSource`. The example has its own Go module so Quark/Orbit never enter the framework's dependency graph, and builds from the Quantum suite workspace (see its README). Replaces the `showcase_demo` removed in the dead-examples cleanup, rebuilt on the fluent builder + module API. (`examples/showcase_demo`)
...
- **BREAKING: `App.Admin` field, `App.MountAdmin()`, and `App.RegisterAdminModels()` removed from `pkg/app`.** These were the only call sites for the in-core admin panel bootstrap. Applications that called them must switch to mounting the `orbit` module (see above). (`pkg/app`)

- **BREAKING: `AdminAgentConfig` type and all flat `admin_*` config keys removed.** The following `nucleus.yml` keys no longer exist in the framework core; under the default strict unknown-field validation (ADR-010 §15) leaving them in `nucleus.yml` now causes a startup error, so they must be removed or moved into the `orbit` module config: `admin_prefix`, `admin_title`, `admin_auth_database`, `admin_bootstrap_username`, `admin_bootstrap_email`, `admin_bootstrap_password`, `admin_live_exclude_patterns`, `admin_cluster_enabled`, `admin_cluster_redis_url`, `admin_cluster_channel`, `admin_cluster_node_id`, `admin_cluster_token`, `admin_trace_url_template`, and the entire `admin:` (cluster agent) config subtree. These keys are now owned by the `orbit` module under its `modules.orbit.*` namespace. Operators upgrading must move these keys into their `orbit` module config and rotate any secrets (`admin_bootstrap_password`, `admin_cluster_token`, `admin_cluster_redis_url`) that appeared in config files or environment variables. (`pkg/app/config.go`)

- **BREAKING: `GET /_/config` endpoint removed (ADR-010 Phase 3b).** The auth-session-gated config introspection endpoint is no longer mounted. The CLI alternative `nucleus config print --effective` (ADR-010 Phase 3a) — backed by `EffectiveConfig`, `EffectiveValue`, and `LoadEffective`, which remain in `pkg/nucleus` — is the supported path for inspecting the resolved configuration. (`pkg/nucleus`)

- **`authz.BootstrapAllowList` no longer auto-exempts `/admin` or `/admin/*`.** The default-deny RBAC allow-list seeded bare-prefix and wildcard rows for the configured admin prefix; with the admin panel extracted, the core RBAC layer has no knowledge of any admin prefix. The `orbit` module self-registers its own prefix exemption at mount time. Call sites that relied on the exemption being present in the core allow-list (e.g. tests that assert on `authz.BootstrapAllowList()` entries) must be updated. (`pkg/authz`)

- **The admin observability cluster subsystem — the `admin/proto`, `admin/agent`, and `admin/server` Go modules together with the `admin/ui` React dashboard — has been removed from the nucleus repository and relocated to the separate `github.com/jcsvwinston/orbit` module (ADR-019).** These components lived outside the nucleus root module, each with their own `go.mod`, wired together via a repo-root `go.work`; they were never importable as `pkg/*` and were not part of the stable API surface, so this removal does not constitute a stable-contract break. As a consequence of this extraction, the repo-root `go.work` and `go.work.sum` files, the proto/UI `Makefile` targets that drove protobuf code generation and the React dashboard build, and the `admin-skeleton` CI job have all been removed. Nucleus is now a single Go module. (`admin/proto`, `admin/agent`, `admin/server`, `admin/ui`)

### Deprecated

- **`admin_rbac_policy_file` config key renamed to `rbac_policy_file`.** The `rbac_policy_file` key feeds the core RBAC enforcer (`pkg/authz`), which is not part of the admin panel and remains in the framework. The old `admin_rbac_policy_file` key is retained as a one-release alias: when `rbac_policy_file` is empty and the deprecated key is set, the framework uses the deprecated value and emits a one-time `WARN` at startup (`admin_rbac_policy_file is deprecated; rename to rbac_policy_file`). The alias will be removed in a future release. Both `pkg/app.Config.RBACPolicyFile` (canonical) and `Config.AdminRBACPolicyFile` (deprecated, zero-value when not set) are present in this release; callers reading the struct should use `RBACPolicyFile`. (`pkg/app/config.go`)

### Security

- **`pkg/observe` default log-redaction set retains defensive entries for the removed `admin_*` secret keys during the migration window.** Even though `admin_bootstrap_password`, `admin_cluster_token`, and `admin_cluster_redis_url` are no longer valid config keys, the log-attribute redaction denylist in `pkg/observe/redact.go` continues to scrub those attribute names. Operators who carry these secrets in environment variables or logging pipelines during a rolling upgrade are protected from accidental secret exposure in structured logs before they complete their migration to the `orbit` module's config namespace. The entries will be pruned once the migration window closes. No behavior change for applications that have already migrated. (`pkg/observe`)

- **Closed admin login username-enumeration timing oracle (fleetdesk finding #17).** The login handler previously took two distinct code paths depending on whether the submitted username existed in the database: a real `bcrypt.CompareHashAndPassword` call for known users versus an immediate return for unknown ones. The two branches produced measurably different response latencies (~100–300 ms gap under load), allowing an unauthenticated caller to enumerate valid admin usernames purely from timing. The unknown-username branch now performs a constant-cost `bcrypt.CompareHashAndPassword` against a pre-computed cost-12 dummy hash, burning the same wall-clock time as the real credential check before returning the failure. Status codes and response bodies are unchanged on both paths — there is no user-visible difference. (Note: this code is now in the `orbit` module; the fix was applied before extraction.) (`pkg/admin`)

### Fixed

- **`router.CSRFToken(r)` now returns the token for any configured session key, fixing session-mode CSRF for forms (fleetdesk finding #27).** In session-token mode the CSRF middleware stores the token under the configurable `CSRFOptions.SessionKey`, but `CSRFToken(r)` read a hard-coded `"csrf_token"` key — so any app that set a custom `SessionKey` got `""` back on first render and could not embed the token in its forms (every subsequent POST then failed CSRF). The middleware now injects the exact token it resolved for the request into the request context, and `CSRFToken(r)` returns that — authoritative across cookie/session storage and any session key, with the previous cookie/session lookup kept as a fallback for callers without the middleware in the chain. This also corrects the finding's original diagnosis: session-mode CSRF *does* work from `Module.Middleware` position (the session is in context before module middleware runs — `injectDependencies` wraps the group chain); the broken first-render token, not middleware ordering, was the real defect. Regression-guarded by `TestCSRFSessionMode_FromGroupMiddleware` and `TestCSRFToken_HonorsCustomSessionKey`. No exported-symbol or signature change. (`pkg/router`)
...
- **`nucleus.Runtime.DatabaseHandle() *db.DB` and `Runtime.DatabaseHandles() map[string]*db.DB` — expose the framework's engine-aware wrapper handles for dialect-aware module use (ADR-019, orbit Slice 2.2 prerequisite).** Deep modules such as the forthcoming `orbit` admin product need the `*db.DB` wrapper — not the underlying `*sql.DB` — to call `db.NewMigrator`, `DB.Engine()`, and `DB.System()` for dialect-aware DDL, migration, and introspection. `DatabaseHandle()` returns the wrapper for the default database; `DatabaseHandles()` returns a snapshot map of every configured alias to its wrapper. Both are additive counterparts to the existing `Runtime.DB()` / `Runtime.Databases()` (`*sql.DB`) accessors. The returned handles are framework-owned — callers must not close them. Both return nil on an unbacked runtime (`DatabaseHandle()` also when no default database is configured). Additive: two new methods on the `Runtime` interface (contract baseline gains `iface-method:Runtime.DatabaseHandle` and `iface-method:Runtime.DatabaseHandles`); nothing is removed or renamed. Existing modules that do not call these methods are unaffected. Backward compatible. (`pkg/nucleus`)

- **`nucleus.Router.Mount(pattern string, h http.Handler)` — lets a pluggable module attach a self-contained `http.Handler` subtree under its route prefix (ADR-019, orbit Slice 2.2 prerequisite).** A module that owns its own router (e.g. the forthcoming `orbit` admin product) can now call `r.Mount("/admin", orbitalRouter)` from inside `Routes(r Router)`. The mounted handler is wrapped with `http.StripPrefix` so that the subtree's internal paths are relative to `pattern`, and any middleware applied via `Router.With` or `Router.Group` at the mount point continues to wrap it. This is the module-facing counterpart to the framework-internal `app.MountAdmin` path, which previously used the same `chi` sub-router mechanism but was unavailable to external modules. Additive: one new method on the `Router` interface (contract baseline gains `iface-method:Router.Mount`); nothing is removed or renamed. `Router` is framework-implemented by contract — existing modules that do not call `Mount` are unaffected. Backward compatible. (`pkg/nucleus`)

- **`nucleus.Runtime.Observability() EventBus` — stable, first-party observability bus accessor for in-process modules (ADR-019, orbit Slice 1c).** In-process pluggable modules (such as the forthcoming `orbit` product's live SQL/HTTP feed) previously had no stable path to the process-wide event stream without importing the experimental `pkg/observability` package. `Runtime.Observability()` returns a `nucleus.EventBus` — a stable interface with two methods: `SubscribeSQL() (<-chan SQLEvent, func())` and `SubscribeHTTP() (<-chan HTTPEvent, func())`. Each subscription returns a typed read channel and a cancel func that closes the channel and stops the backing goroutine; the caller must invoke the cancel func. `nucleus.SQLEvent` and `nucleus.HTTPEvent` are first-party value types (detached copies): the adapter handles the bus's `Release`/refcount discipline internally, so consumers own plain values and carry no reference to `pkg/observability` types. Slow consumers drop events rather than blocking the bus (backpressure). The firewall is deliberate (ADR-019 "Option A2"): no `pkg/observability` type crosses the `pkg/nucleus` boundary. Additive: one new method on the `Runtime` interface (baseline gains `iface-method:Runtime.Observability`) plus three new exported types (`EventBus`, `SQLEvent`, `HTTPEvent`); nothing is removed or renamed. `Runtime` is framework-implemented by contract — existing modules are unaffected. Backward compatible. (`pkg/nucleus`)

- **`nucleus.Runtime.Models()` and `nucleus.Runtime.Databases()` — new accessors enabling in-process pluggable modules to enumerate the application's model registry and all managed database handles (ADR-019, orbit Slice 1a).** `Models() *model.Registry` returns the shared registry that the framework populates with every module's `Models()` contribution, enabling a pluggable module (such as the forthcoming `orbit` admin product) to enumerate and perform CRUD over arbitrary application models without coupling to any specific one. `Databases() map[string]*sql.DB` returns a snapshot copy of every configured managed database handle keyed by alias — unlike `Runtime.DB()`, which is bound to a single module-declared alias, this covers all aliases in a multi-database or multi-tenant topology; the handles are framework-owned and must not be closed by the caller. Both methods sit on the `Runtime` interface, which is framework-implemented by contract; existing module authors are unaffected (no removal or rename). Additive: contract baseline gains two lines (`iface-method:Runtime.Models`, `iface-method:Runtime.Databases`). Backward compatible — existing `Runtime` call sites require no change. (`pkg/nucleus`)

- **`pkg/authz` — `MiddlewareWithOptions` now accepts pluggable subject and action resolvers via two new `AuthzOptions` fields (fleetdesk finding #35).** `ResolveSubject` (`authz.SubjectResolver` — `func(r *http.Request, claims *auth.Claims) string`) lets a policy table that is keyed on role rather than user ID substitute `claims.Role` (or any derived string) as the Casbin subject; `ResolveAction` (`authz.ActionResolver` — `func(r *http.Request) string`) lets an SSR app that tunnels destructive operations through `POST` supply a logical action (e.g. `"delete"`) rather than the HTTP-method default. Both fields are optional: when nil the previous defaults apply — `claims.UserID` as subject, the HTTP-method mapping as action — so existing `Middleware()` and `MiddlewareWithOptions` call sites (including the `WithOptions`-backed `Middleware()`) are byte-for-byte equivalent in behaviour. The `authz denied` structured log entry now also emits a `subject` field (the resolved subject string), which is distinct from the `user` field when `ResolveSubject` is set. Purely additive; backward compatible — no existing call site requires any change. (`pkg/authz`)

- **`pkg/authz` now supports SSR-friendly denial rendering via opt-in `AuthzOptions` (fleetdesk finding #26).** `(*authz.Enforcer).Middleware()` and `(*authz.Enforcer).RequireRole(...)` previously only emitted a JSON 401/403 envelope on denial, making them unusable in server-rendered apps that need a styled HTML page or a login redirect. Two new methods — `(*authz.Enforcer).MiddlewareWithOptions(opts AuthzOptions)` and `(*authz.Enforcer).RequireRoleWithOptions(opts AuthzOptions, roles ...string)` — accept an `AuthzOptions{OnDeny DenialHandler}` callback. `DenialHandler` receives an `authz.Denial{Status int, Authenticated bool, Reason string}` value and takes full control of the response, enabling per-route HTML pages, redirects, or any custom envelope. The original `Middleware()` and `RequireRole(...)` are unchanged — they now delegate to the `WithOptions` variants with a zero `AuthzOptions`, producing the byte-for-byte identical JSON 401/403 response as before. Purely additive; backward compatible — existing call sites require no change. (`pkg/authz`)

- **`nucleus.Router.With(mw ...Middleware) Router` — per-route middleware on the fluent router (fleetdesk finding #24).** A module's `Routes(r Router)` closure could mount handlers but had no way to attach a `func(http.Handler) http.Handler` middleware to a *single* route — `Router` had `Get`/`Post`/…/`Group`/`Resource` but no per-route `With`/`Use`. Guarding one endpoint with a framework http-middleware (`Enforcer.RequireRole`, `router.CSRFMiddleware`) therefore meant writing a per-app adapter. `With` returns a Router (an inline sub-scope via `router.Mux.With`) whose middleware applies only to routes registered on it — isolated from sibling and parent routes, composing additively across nested `With`/`Group` layers: `r.With(rt.Authorizer().RequireRole("admin")).Get("/billing", billing)`. Additive: one new method on the `Router` interface (baseline gains `iface-method:Router.With`, nothing removed); `Router` is framework-implemented and module-consumed, so module authors are unaffected. Backward compatible. (`pkg/nucleus`)

- **`pkg/openapi` can now declare security schemes and requirements (fleetdesk finding #33).** A generated contract could describe paths and schemas but had no way to say an endpoint requires authentication — there was no security-scheme support on `Document`/`Components`/`Operation`, so a bearer-authenticated API (e.g. the fleetdesk prototype's `/api/*`) produced an `/openapi.json` that silently omitted the `Authorization` requirement. Added the OpenAPI 3.1 `SecurityScheme` and `SecurityRequirement` types, a `Components.SecuritySchemes` map, document-level `Document.Security`, and operation-level `Operation.Security` (a pointer, so an explicit empty override survives JSON marshalling), plus helpers `BearerAuthScheme`, `APIKeyScheme`, `AddSecurityScheme`, `Require`, `RequireSecurity` and `PublicSecurity` (the empty override that marks an operation public even when the document sets a global requirement). Purely additive and `omitempty` throughout — a contract that sets none of these marshals byte-for-byte as before. `pkg/openapi` is `experimental` (not in the API freeze), so no baseline change. (`pkg/openapi`)

- **`nucleus.Runtime.JWT()` — fluent module accessor for the app's JWT manager (fleetdesk finding #32).** Modules that mint or verify bearer tokens — a token-issuing endpoint, a per-route bearer-auth middleware — previously had no path to the `*auth.JWTManager` the framework builds from `jwt_secret` / `jwt_keys[]`, so they had to construct a duplicate manager from a separately-configured secret. `JWT()` returns the same manager the app assembled (the one whose JWKS the framework auto-mounts when an asymmetric key is configured), so a module signs and validates against the framework's single configured keyset rather than a copy. It degrades to nil on an unbacked runtime and when no signing material is configured (`App.JWT` is nil — a read-only service that only consumes externally-minted JWTs may leave it unset, so callers must guard). Additive: one new interface method on `Runtime`; the contract baseline gains one line (`iface-method:Runtime.JWT`) and nothing is removed. Completes the stable Runtime service-accessor set (Session/Authorizer/Mailer/Storage/JWT). The returned manager is the app-global instance and is mutable — its godoc documents that `RotateKey`/`RemoveKey` are operator-level key lifecycle, not per-request module calls (the same posture as `Authorizer()`). Backward compatible — existing modules and call sites are unaffected. (`pkg/nucleus`)

- **`nucleus.Runtime.Mailer()` and `nucleus.Runtime.Storage()` — fluent module accessors for the app's managed mail sender and object store.** Modules that need to send mail or write/read object-store artifacts (signal handlers, task workers, report exporters) previously had no path to the `mail.Sender` and `storage.Store` that the framework built from `mail_*` / `storage_*` config, which forced them to construct duplicate instances that bypassed the framework's health check, circuit breaker, and background-cleaner lifecycle. `Mailer()` returns the same `mail.Sender` the app assembled — including its circuit-breaker wrap for non-noop drivers — so modules share the single outbound pipeline. `Storage()` returns the same `storage.Store`, with its signed-URL provider and background cleaner. Both degrade to nil on an unbacked runtime and when the respective subsystem was not attached (`app.WithoutDefaults()`); the default mail driver is a no-op sender, so a non-nil `Mailer()` is safe to call even without SMTP configured. Additive: two new interface methods on `Runtime`; contract baseline gains two lines and nothing is removed. Backward compatible — existing modules and call sites are unaffected. (`pkg/nucleus`)
...
- **`auth.SessionInfo`, `(*auth.SessionManager).ActiveSessions`, and `auth.ErrSessionStoreNotIterable` — session-enumeration surface for trusted operator tooling (ADR-019, orbit Slice 1b).** `auth.SessionInfo{Token string, Deadline time.Time, Values map[string]any}` is a decoded, scs-type-free snapshot of one stored session. `(*auth.SessionManager).ActiveSessions(ctx) ([]SessionInfo, error)` enumerates every live session in the store, decodes each payload via the manager's codec, and returns the snapshots; undecodable payloads are skipped rather than returned as errors. Stores that cannot enumerate their full key-space (e.g. cookie stores) return `auth.ErrSessionStoreNotIterable`, and a nil/zero-initialised manager returns `auth.ErrNilSessionManager`, allowing callers to branch cleanly. The intended consumer is a trusted operator surface such as orbit's "active sessions" viewer — callers should treat `SessionInfo.Token` and `SessionInfo.Values` as sensitive. Purely additive; no existing `auth` symbol is removed or renamed. Backward compatible — existing `SessionManager` call sites require no change. (`pkg/auth`)

- **`pkg/admin.Panel.ConsumeObservability(bus)` — admin live SQL feed now consumes the application observability bus, surfacing every `model.CRUD` query across the whole application (ADR-018).** Previously the live view's SQL feed was populated only by a per-CRUD observer installed on the Data Studio's own browsing instances; application queries issued by REST resource handlers and app-side CRUD were invisible in the panel. `ConsumeObservability` subscribes the panel to the process-wide `pkg/observability.Bus` (filter: `KindSQLStatement`), runs a drain goroutine into the existing SQL ring buffer and live event stream, and returns an idempotent stop function also called by `Panel.Close`. `app.New` wires this automatically after panel construction, so no caller change is required. When the bus is connected, `getCRUD` skips the now-redundant per-CRUD observer, so admin-issued queries continue to appear exactly once. Panels built outside `app.New` retain the per-CRUD observer as a fallback. Note: direct `*sql.DB` queries that bypass `model.CRUD` are still not captured (planned follow-up). Additive: one new exported method on `Panel`; `pkg/admin` carries no frozen contract baseline. Backward compatible — no existing method removed or renamed; call sites that do not call `ConsumeObservability` observe no change in behaviour. (`pkg/admin`, `pkg/app`)
