...
- **BREAKING (`pkg/nucleus` rewrite — Phase 1 Foundation of ADR-010): canonical `App{}` struct + `Module[C any]` generic + `Router` interface replace the legacy flat builder chain.** `pkg/nucleus.New().Port().SQLite().Model().AutoMigrate().Run()` no longer exists. The new fluent surface is `nucleus.New().FromConfigFile(...).Use(...).Mount(module.Build()).Start()` (with `Serve()` as a synonym). Two additional coexisting entry points produce the same `nucleus.App` value: direct-struct `nucleus.Run(nucleus.App{...})` and the user-space `bootstrap.New()` convention. Module authors define a typed `Module[C]` with `Routes func(Router, C)`, `Migrations fs.FS`, and lifecycle hooks; `Module[C].Build()` returns the type-erased `ModuleSpec` the framework stores. The builder terminator is `Start()` (not `Run()`) to avoid colliding with the package-level `nucleus.Run(App)` function (Go does not overload). `Router.Resource(prefix, controller, nucleus.Methods(Index, Show, Create))` requires explicit method declarations — silent presence-based discovery is rejected. `WithoutDefaults()` and `WithExtensions(...)` are exposed on the chain so the fluent surface no longer misses the seam `pkg/app` already had. The two example consumers (`examples/ecommerce_dashboard/backend/{main.go,handlers/handlers.go}` plus a new `module.go`) are updated in this PR per CLAUDE.md §3. Pre-`v1.0` clean break per the ADR-006 / ADR-008 precedent — no `DEP-` / `MA-` artefacts. Phases 2-4 (config merge engine, effective-config inspection, docs-sync mechanism + website rewrite) follow in separate PRs. See [ADR-010](docs/adrs/ADR-010-fluent-api-v2-pkg-nucleus.md).
