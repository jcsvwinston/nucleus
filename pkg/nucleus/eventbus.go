...
//
// It deliberately exposes only the subscribe operations a consumer needs and
// emits nucleus-owned event VALUES (SQLEvent/HTTPEvent), not the bus's pooled,
// refcounted event objects. So a module never imports the experimental package,
// is insulated from its pre-v1.0 churn, and is freed from the bus's Release
// discipline — the adapter performs the required Release internally and hands
// the consumer a detached copy it owns outright.
//
