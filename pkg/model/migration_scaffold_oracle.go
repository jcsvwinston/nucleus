...
//
//   - Identifier quoting uses double quotes (`"name"`). Unquoted Oracle
//     identifiers are folded to uppercase at parse time; quoting
//     preserves the case used in the model.
...
func BuildOracleMigrationScaffold(meta *ModelMeta) (string, string, error) {
	...
	for _, f := range meta.Fields {
		...
		if f.IsPK {
			if oracleAutoIncrementPKType(f.GoType) {
				def = quoteOracleIdentifier(column) + " NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
			} else {
				def = quoteOracleIdentifier(column) + " " + oracleTypeForField(f) + " PRIMARY KEY"
			}
		} else {
			def = quoteOracleIdentifier(column) + " " + oracleTypeForField(f)
			if f.IsRequired {
				def += " NOT NULL"
			}
		}
	...
	for _, fk := range meta.ForeignKeys {
		...
		columnDefs = append(columnDefs,
			fmt.Sprintf(
				...
				quoteOracleIdentifier(constraintName),
				quoteOracleIdentifier(column),
				quoteOracleIdentifier(foreignTable),
				quoteOracleIdentifier(foreignColumn),
			),
		)
	}
	...
	createTable.WriteString(quoteOracleIdentifier(table))
	...
	for _, idx := range meta.Indexes {
		...
		for _, col := range idx.Columns {
			...
			quotedCols = append(quotedCols, quoteOracleIdentifier(col))
		}
		prefix := "CREATE INDEX"
		if idx.Unique {
			prefix = "CREATE UNIQUE INDEX"
		}
		stmt := fmt.Sprintf("%s %s ON %s (%s)", prefix, quoteOracleIdentifier(name), quoteOracleIdentifier(table), strings.Join(quotedCols, ", "))
	...
	for i := len(indexNames) - 1; i >= 0; i-- {
		writeOraclePLSQLBlock(down, "DROP INDEX "+quoteOracleIdentifier(indexNames[i]), "-1418", "specified index does not exist")
	}
	writeOraclePLSQLBlock(down, "DROP TABLE "+quoteOracleIdentifier(table), "-942", "table or view does not exist")
