...
**Follow-on (owner clarification 2026-05-25) — DONE:** moving the demo to
embedded files was still "el mismo perro con distinto collar" — a hardcoded demo
in the core. Owner's actual principle: the framework core must carry NO example.
So `nucleus new` now generates a minimal EMPTY SKELETON (no demo feature code);
the working reference app lives ONLY in `examples/mvc_api`. Committed `f073953`
(skeleton scaffolder) + `555bd1d` (docs/CHANGELOG aligned). The whole
scaffolder-cleanup arc is COMPLETE and pushed.

## Most recent completed iteration

- **Scaffolder skeleton-ification — no hardcoded demo in core** (2026-05-25,
  COMPLETE — committed `f073953` + `555bd1d` docs/CHANGELOG; pushed). Removed the
  entire `articles` demo from the embedded templates; both `api`/`mvc` templates
  now emit a minimal skeleton (composition-root `main.go` with no Mount, config,
  README→examples, empty `migrations/`; mvc keeps a 1-line `/healthz` RBAC grant).
  Verified by RUNNING both skeletons (api/mvc `/healthz` 200, mvc `/admin` 302, no
  demo routes); running it caught + fixed a stale `redis_url` causing `/healthz`
  503. quickstart/project-structure re-framed (skeleton + worked example from
  examples/mvc_api). This settles the "articles vs notes" naming question (the demo
  is simply gone from core).
...
- **COMPLETE.** Scaffolder-cleanup arc finished: string-demo → embedded files →
  empty skeleton (no hardcoded example in core). All pushed to origin/main
