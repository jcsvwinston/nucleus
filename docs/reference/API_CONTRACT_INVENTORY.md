...
| `pkg/nucleus` | `stable` | `App{}` canonical struct embedding `app.Config` plus Go-only wiring fields (`Modules map[string]ModuleSpec`, `Middleware []Middleware`, `Services []ServiceRegistration`, `Lifecycle LifecycleHooks`, `Options []Option`); `New() *AppBuilder` and `AppBuilder` chain (`FromConfigFile`, `Use`, `Mount`, `WithoutDefaults`, `WithExtensions`, `WithConfigStrict`, `Build`, `Start`/`Serve`, `Err`); package-level `Run(App) error` direct-struct surface; generic `Module[C any]` with `Build() ModuleSpec`; `Router` interface (`Get`, `Post`, `Put`, `Patch`, `Delete`, `Group`, `Resource`); `Methods(...) MethodSet`; REST verb constants (`Index`, `Show`, `Create`, `Update`, `Patch`, `Destroy`) and sub-interfaces (`Indexer`, `Shower`, `Creator`, `Updater`, `Patcher`, `Destroyer`); `Context`/`Handler`/`Middleware`/`Option`/`Extension` types; config-loader surface — `MaxConfigFileBytes`, sentinels `ErrConfigFileTooLarge`, `ErrUnsupportedConfigFormat`, `ErrUnknownConfigKeys`, `ErrSecurityKeyNotNullable`, `ErrMixedConfigFormats` | Fluent façade over `pkg/app` (ADR-010). Three coexisting entry surfaces — fluent builder, direct struct, bootstrap pattern — produce equal `App{}` values per `pkg/nucleus/equivalence_test.go`. `FromConfigFile` is the Phase 2 loader (Phase 2a: single-file YAML + size cap + strict schema; Phase 2b: TOML/JSON + multi-file merge with `_append`/`_remove` operators + null-revert-to-default + non-nullable security keys + `WithConfigStrict(true)` mixed-format rejection). `Resource` registration is auditable — controllers must explicitly satisfy each requested verb's sub-interface or the framework panics at startup (no reflection-based method discovery). |
| `pkg/db` | `stable` | `db.New`, `db.DB` (incl. `Health`, `System`), migrator APIs (`NewMigrator`, migration lifecycle methods, file-level `Drift`/`DriftEntry`/`DriftKindMissingUpFile`/`DriftKindChecksumMismatch`, schema-level `SchemaDrift`/`SchemaDriftEntry`/`ExpectedTable`/`ExpectedColumn`/`DriftKindSchemaMissingTable`/`DriftKindSchemaMissingColumn`/`DriftKindSchemaExtraColumn`/`DriftKindSchemaColumnNullability`/`ErrSchemaDriftUnsupported`), SQL URL support | URL schemes `sqlite://`, `postgres://`/`postgresql://`, `mysql://`, `sqlserver://`/`mssql://`, `oracle://` are all `stable` (MSSQL/Oracle promoted to required CI gate 2026-05-12). `SchemaDrift` introspects the live schema for all five supported engines (SQLite, PostgreSQL, MySQL, MSSQL, Oracle — ADR-009 addendum 2026-05-15). `ErrSchemaDriftUnsupported` fires only for engines outside the supported set. |
