...
| `pkg/circuit` | `stable` | `Config`, `New`, `Breaker` (`Do(ctx, fn)`, `State()`), `State` enum, `ErrOpen` | Standalone three-state circuit breaker; race-tested. Operators opt in by wrapping external calls (mail, storage, plugin bridges, third-party APIs). Not auto-wired. |
