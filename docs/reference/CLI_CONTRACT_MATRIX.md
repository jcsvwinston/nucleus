...
| `routes` | `stable` | Route introspection contract; `--json` is automation-safe. Since 2026-09 the command reads the compiled binary: inside a project (a `go.mod` in `--dir`, default `.`) it runs `go run .` with `NUCLEUS_PRINT_ROUTES=1`, which makes `nucleus.Run` print its route table and exit before listening, and lists every route with the module that registered it (`module` column/field; empty for the framework's own). `--framework-only` (additive) keeps the configuration-only listing, which is also the fallback outside a project — both declare themselves in a note. Plain output gained a third tab-separated column (`METHOD\tPATTERN\tMODULE`); existing consumers reading the first two columns are unaffected. |
...
| `openapi` | `experimental` | Exports the project OpenAPI JSON document from `internal/contracts`; generated runtime serving should use that same document builder and remains an explicit `MountOpenAPIHandler(...)` application decision, and the current subset includes scaffolded JSON request/response metadata, shared `data`/`count` response envelopes, shared JSON error/empty responses, and explicit path/query parameters where declared. |
