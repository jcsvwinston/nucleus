...
| `routes` | `stable` | Route introspection contract; `--json` is automation-safe. Since 2026-09 the command reads the compiled binary: inside a project (a `go.mod` in `--dir`, default `.`) it builds the main package and runs the binary with `NUCLEUS_PRINT_ROUTES=1`, which makes `nucleus.Run` print its route table and exit before listening, and lists every route with the module that registered it (`module` column/field, present on every JSON entry and empty for the framework's own; a subtree a module registers through `Router.Group` is one `<prefix>/*` entry, the same fidelity as the boot log). The run is bounded by `--timeout` (default 30s; the build is not counted): an application that serves instead of printing is stopped with its process group and reported as an error. A project whose nucleus requirement predates the variable is not built; the command lists the framework routes from the project's config file and says so. `--config` applies to the configuration-only listing only: inside a project it is refused (non-zero) unless `--framework-only` is given, never silently ignored. `--framework-only` (additive) keeps the configuration-only listing, which is also the fallback outside a project — both declare themselves in a note that carries the phrase `listing framework-owned routes only`. Plain output gained a LAST tab-separated column for the module: `METHOD\tPATTERN\tMODULE`, and with `--verbose` `METHOD\tPATTERN\tmiddleware=N\tMODULE` — every column that existed before keeps its position. |
