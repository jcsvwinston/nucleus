...
| `new` | `stable` | Project scaffold entrypoint contract. `--db <engine>` (additive, 2026-09) selects the driver module the generated `main.go` imports and the `databases.default.url` it starts on; the scaffold runs `go get <driver>` and `go mod tidy` so the project builds without a manual tidy, and `--offline` skips both (the default for the repository's own tests). `--with <orbit,quark,quarkbridge,quarkdatasource>` and `--template suite` (additive, 2026-09): the suite siblings are fetched from the module proxy at their published tags with the same `go get` step (no sibling version is pinned in the CLI; the framework module never requires a sibling), `--with orbit` mounts the admin panel in the generated `main.go`, a sibling the template does not import is fetched after the tidy so it stays in `go.mod` as an indirect require, and `--template suite` — which implies all four — writes the suite wired together (`shop` module on Quark with its own policy rows and test, Orbit under `/admin`, both bridges). `examples/showcase_demo` is the committed output of the suite template; `TestShowcaseDemoMatchesSuiteTemplate` fails when they differ and `make regen-showcase` rewrites the example. The only boot of the suite scaffold in this repository is `TestSuiteScaffoldBootsWithSiblingCheckouts`, skipped without orbit/quark checkouts next to the repo; the umbrella's workspace lane is the CI boot. |
...
| `completion` | `experimental` | Shell completion generator (2026-09): `completion <bash\|zsh\|fish>` prints a script generated from `commandSpecs`, `commandAliases`, each command's flags (read from its own `--help`) and its `usageSpec` grammar (the subcommands a first positional accepts; the values a flag accepts, from a usage section titled `... (--flag)`), so an offered word is one the binary accepts. Commands whose help has no `flag.PrintDefaults` block (`add`, `plugin`, `outbox`, `config`) complete their name only; `generate` completes its flags (its help ends with a `PrintDefaults` block) but not its `<kind>` words, which come from prose it prints by hand, not from a `usageSpec`. The goldens are regenerated with `go test ./internal/cli -run TestCompletionGolden -update` after any flag change. Golden files under `internal/cli/testdata/completion/` pin the three scripts. The script is a snapshot of one binary; regenerate after upgrading. No compatibility guarantee on the script text. |
