...
A Nucleus application's HTTP server is compiled from the project-root `main.go`
(the composition root generated by `nucleus new`). A separate worker process
(`cmd/worker`) is optional — add one only if your project uses background tasks.
Both can share the same image but run as separate containers/processes.
...
```bash
# Build server (project-root composition root)
go build -o bin/server .

# Build worker (only if your project includes cmd/worker)
...
COPY . .
# Build the server from the project-root composition root
...
RUN go mod download
COPY . .
# Build server from the project-root composition root
RUN CGO_ENABLED=0 go build -o /bin/server .
# Build worker only if your project includes cmd/worker
# RUN CGO_ENABLED=0 go build -o /bin/worker ./cmd/worker
