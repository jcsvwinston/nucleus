...
Prometheus / OpenMetrics exposition of the OTel MeterProvider's measurements. Mounted at the path configured by `metrics_path` (default `/metrics`); set `metrics_path: ""` in `nucleus.yml` to disable. Content-Type is `application/openmetrics-text`. By default the endpoint answers without authentication (`metrics_public: true`): restrict access at the network / reverse-proxy layer (allow-list your scraper), or set `metrics_public: false` to gate it behind the default-deny RBAC enforcer (your scraper then needs an explicit policy grant).
