...
**Status:** Accepted (Phase 1 landed 2026-05-16; Phase 2a landed 2026-05-16; Phase 2b landed 2026-05-17; Phase 2c landed 2026-05-17; Phase 2d landed 2026-05-17; Phase 3a landed 2026-05-22)
**Date:** 2026-05-15
**Accepted:** 2026-05-16
...
- **Phase 3 — Effective-config inspection.** Compliance items #6, #12, #13.
  ...
  - **Phase 3b (pending):** the auth-gated `/_/config` endpoint (compliance #12). Note: §5 / #12 assume a `WithAdmin()` composition toggle that does not exist — admin mounts as a default subsystem (gated by `WithoutDefaults()`), so 3b will gate `/_/config` on the admin subsystem being active (`App.Admin != nil`) and wrap it with `admin.NewDatabaseAdminAuth(App.DefaultDB(), App.Session, App.Config.AdminPrefix)` over the app-wide Casbin default-deny. The effective-config snapshot must be threaded from the builder into `Run`.
