...
An authentication-gated `/_/config` endpoint was added in Phase 3b to expose the same view at runtime (see Phase 3b Implementation phases entry). It was subsequently removed by ADR-019 (2026-06-21) when the admin subsystem was extracted to the `orbit` module; the endpoint was session-gated via `pkg/admin` and left with that package. `nucleus config print --effective` remains the canonical inspection path.

**Auth gate (historical, Phase 3b only).** `/_/config` was gated by the same admin-panel authentication that guarded `pkg/admin` routes (session-based, subject to the Casbin default-deny established by ADR-004). It was mounted only when the admin subsystem was active. Removed ADR-019 2026-06-21.
...
- **Phase 2 — Config loading + merge engine.** Compliance items #4, #7, #14, #15, #16, #17.
  ...
  - **§2 layer 5 — module-specific config binding/validation (landed 2026-05-29):** the fifth and final validator layer (new `validate_module_config.go`, new `ErrInvalidModuleConfig` sentinel). Each mounted module's `modules.<name>.*` subtree is bound into its typed `Module[C].Config`, `default:` struct tags fill still-zero fields, and `validate:` tags are enforced via `pkg/validate` (go-playground/validator). **Loader:** the `modules.*` namespace is exempted from the layer-2 unknown-key guard in the nucleus loader (`stripModuleConfigKeys` — NOT added to `app.ContractConfigKeyPatterns()`, since the namespace is open-ended and each module owns its own schema via its tags) and the per-module subtree is sliced out of the merged koanf (`extractModuleConfigs`) and retained on the unexported `App.moduleConfigsRaw`. **Binding point:** like layer 4's module half, binding runs in `Run` (`bindModuleConfigs`), not `FromConfigFile`/`Mount` — those two may be called in either order, so only at `Run` are both the merged config and the full module set known. It runs before `app.New` so a bad module config fails fast. **Both paths:** the builder path binds from the file subtree; the direct-struct `Run(App{})` path (no file) applies `default:` tags + validation to the programmatic config only, mirroring layers 3/4. The bound specs replace `App.Modules` (a fresh map, so the direct-struct caller's map is never mutated) before `registerModuleModels`/`OnStart`/`Routes`. **`default:` tags** are applied by a small in-package reflection helper (`applyDefaults`) — no new dependency (stdlib-first); supports scalars, `time.Duration`, and nested structs, and fills only zero-valued fields (an author/file value always wins). Config for an unmounted module is a non-fatal WARN (a strict reject would couple `Run` to the `FromConfigFile` unknown-fields mode, which is not threaded that far). Out of scope (deferred): env-layer override of the `modules.*` subtree (`applyEnvLayer` only applies schema-recognised keys, so `NUCLEUS_MODULES__*` vars are ignored today). Contract: `ErrInvalidModuleConfig` rebaselined additively (+1); no removed/renamed symbol.
- **Phase 3 — Effective-config inspection.** Compliance items #6, #12, #13.
  ...
  - **Phase 3b (landed 2026-05-23):** the auth-gated `GET /_/config` endpoint (compliance #12), the runtime mirror of `config print --effective`. As-built: §5 / #12 assumed a `WithAdmin()` composition toggle that does not exist, so the endpoint is mounted from the nucleus layer (`pkg/nucleus.Run`) and gated on the admin subsystem being active (`core.Admin != nil`) — a `WithoutDefaults()` app does not expose it. Three defence-in-depth layers: (1) the mount gate; (2) the app-wide ADR-004 Casbin default-deny — since that enforcer reads JWT claims only, a session-admin resolves to the `anonymous` subject there, so `Run` adds a bootstrap-subject allow policy for the fixed `/_/config` path via `core.Authorizer.AddPolicy(authz.BootstrapSubject, "/_/config", "*")` (the same precedent as `/admin/*` in `authz.BootstrapAllowList`, but added from the nucleus layer that owns the route rather than by editing the stable `pkg/authz` package); (3) `admin.NewDatabaseAdminAuth(core.DefaultDB(), core.Session, core.Config.AdminPrefix)` validates the admin session — anon → 403, admin session → 200 JSON (`Cache-Control: no-store`). The effective snapshot is threaded builder→`Run` via an unexported `App.effective *EffectiveConfig` captured in `FromConfigFile` (with the same load options + the app's `LogRedactExtraKeys`, so the endpoint's redaction is app-aware where the CLI is canonical-only). The direct-struct `Run(App{})` path (no file paths) falls back to a snapshot flattened from the live `core.Config` with a new `ConfigSource.Kind` value `"runtime"` (alongside the file kinds `"default"`/`"yaml"`/`"toml"`/`"json"`). Redaction reuses the canonical `observe.DefaultRedactedKeys()`, extended in this PR with the AWS access-key-ID pair (`access_key_id`, `aws_access_key_id`) so the S3 credential is fully covered; public identifiers (`account_name`, `smtp_user`, admin bootstrap username/email) are deliberately left in cleartext.
    > **Amendment (2026-06-21) — ADR-019 admin extraction (Slice 2):** `GET /_/config` was removed as part of the clean-break extraction of the in-core admin panel to the separate `orbit` Go module. The endpoint was admin-session-gated (`core.Admin != nil` mount gate + `admin.NewDatabaseAdminAuth` session check); with `pkg/admin` extracted, the authentication backing it no longer exists in core. The Phase 3a CLI surface (`nucleus config print --effective` — `pkg/nucleus.LoadEffective`) is unaffected and remains the canonical way to inspect effective configuration. ADR-010 status is unchanged (Accepted).
