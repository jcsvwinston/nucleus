...
func controls() []control {
	return []control{
		...
		{id: "JOB-09", family: "queue", title: "what is pending, running and failed is visible",
			want: partial, note: "since S1 the snapshot carries a queue row and the held count, but pending and active are still zero whatever the queue holds: the provider does not track them, and a zero claims the queue is empty rather than admitting it is not counted (NU-82, A7 S3)",
