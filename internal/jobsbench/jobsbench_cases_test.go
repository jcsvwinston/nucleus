...
func controls() []control {
	return []control{
		...
		{id: "RT-04", family: "realtime", title: "one message can be pushed to everyone on a topic",
			want: present, probe: probeChannelBroadcast},
		{id: "RT-05", family: "realtime", title: "joining a topic is authenticated like a route",
			want: present, note: "the handler decides before serving, exactly as it does for any other route, and what it decides travels with the connection as the client's identity — a channel does not get a second authorisation mechanism to drift from the first",
			probe: probeChannelAuth},
		{id: "RT-06", family: "realtime", title: "who is connected to a topic can be asked",
			want: present, note: "one entry per CONNECTION, not per person: two tabs of one user are two entries, which is what a device list needs",
		...
		{id: "OPS-01", family: "ops", title: "liveness has an endpoint of its own",
			want: present, note: "/livez checks nothing but the process: a liveness probe that fails when the database is down asks the orchestrator to restart every replica at once over a dependency a restart cannot fix",
			probe: probeLiveness},
		{id: "OPS-02", family: "ops", title: "readiness has an endpoint of its own",
			want: present, probe: probeReadiness},
		...
		{id: "OPS-04", family: "ops", title: "a profiler is reachable and not public",
			want: present, note: "off by default and never in the bootstrap allow-list: the profiles carry live process memory, so with profiling_enabled they answer 403 to anyone the application's policy does not name",
		...
		{id: "OPS-06", family: "ops", title: "a sqlite application waits instead of failing under contention",
			want: present, probe: probeSQLiteBusyTimeout},
		{id: "OPS-07", family: "ops", title: "a module can create its schema before the outbox starts reading",
			want: present, note: "delivery begins with StartOutbox, which the runtime calls once the modules are up; an application built with app.New alone gets it from Run. Enqueueing still works before then — it is delivery that waits, not acceptance",
