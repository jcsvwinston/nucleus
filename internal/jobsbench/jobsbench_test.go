...
// Package jobsbench is the measured inventory of what Nucleus can do today
// with background jobs, events and real-time channels — one executable probe
...
// one runs: it boots an application and asks the route, drives the public API
// of pkg/tasks, pkg/signals or pkg/outbox and checks the answer, or asks the
// configuration layer for a key and reads the error it gets back.
//
...
const (
	...
	// a booted application, a rejected configuration key, an API that drops
	// what it was handed — never the lack of a grep hit.
	absent verdict = "absent"
)

// control is one capability of the jobs/events/real-time surface, with the
// probe that decides its verdict.
type control struct {
...
func TestJobsBench(t *testing.T) {
	cases := controls()
	...
	for _, c := range cases {
		t.Run(c.id, func(t *testing.T) {
			...
			t.Errorf("control %s (%s) measures %q, the bench records %q.\n\n"+
				...
				"recorded verdict in jobsbench_cases_test.go in the same change, so\n"+
...
// TestJobsBenchSummary prints the table the arc plan quotes. It asserts
// nothing: TestJobsBench is what fails when a verdict drifts.
func TestJobsBenchSummary(t *testing.T) {
	cases := controls()
