...
func controls() []control {
	return []control{
		...
		{id: "MAIL-02", family: "mail", title: "send an HTML / multipart message",
			want: absent, note: "Message carries one Body; verification and reset mail is HTML everywhere this arc copies from",
			probe: probeMailHTML},
		{id: "MAIL-03", family: "mail", title: "attachments",
			want: absent, note: "no field", probe: probeMailAttachments},
		{id: "MAIL-04", family: "mail", title: "render a message from a template",
			want: absent, note: "no field", probe: probeMailTemplates},
		{id: "MAIL-05", family: "mail", title: "queued mail survives a crash",
			want: partial, note: "pkg/outbox exists as a general mechanism; the mail sender does not go through it, so a verification email is lost between the database write and the SMTP call",
			probe: probeMailDurability},
