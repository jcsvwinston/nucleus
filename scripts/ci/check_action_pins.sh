#!/usr/bin/env bash
# check_action_pins.sh — every GitHub Action the nine workflows under
...
# whatever the owner of that tag decides to publish under it, in a job that has
# this repository checked out. Several of those jobs are not cheap targets —
# release.yml signs artifacts with cosign and attests provenance, scorecard.yml
# and codeql.yml upload SARIF — so "which code ran in that release" has to stay
# answerable after the fact. A 40-hex SHA is immutable, which is what OpenSSF
# Scorecard's Pinned-Dependencies check asks for and what makes that question
# answerable.
#
...
# github-actions ecosystem is declared in .github/dependabot.yml, grouped so a
# bump arrives as one PR). A pin without a bot behind it is worse than a tag —
# it freezes the action on the day it was written, security fixes included — so
# the comment and the Dependabot entry are part of the rule, not a nicety.
#
# The pins were placed by hand (PR #492) and held by review alone until this
# guard existed. The next hand-written `uses: actions/checkout@v4` is what it
# is here to catch.
#
...
while IFS= read -r line; do
  ...
  # party code running in this repository's context exactly like an action, and
  # Dependabot bumps it through the same github-actions ecosystem. This repo
  # has none today; the rule is here so the first one arrives pinned.
