...
| `nucleus doctor --check image` | Read the project's `Dockerfile` and report high-risk container settings: a runtime stage that runs as root, a base image on a mutable tag, a build with cgo left on, a whole-context `COPY` into the runtime stage, credentials baked into `ENV`/`ARG`, and a missing `.dockerignore`. A project with no `Dockerfile` is reported as not applicable, not as a problem. |
