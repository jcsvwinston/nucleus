...
| ----------------------------- | ----------------------------------------------------- |
| `nucleus new <name>`          | Scaffold a new project (`--template mvc\|api`, `--db sqlite\|postgres\|mysql\|sqlserver\|oracle`). The driver module for `--db` is imported by the generated `main.go` and resolved on the spot (`go get` + `go mod tidy`), so the project builds as written; `--offline` skips both and hands them back as the next step. |
...
| `nucleus generate module <name>`   | Scaffold the same feature as **one** self-contained package under `internal/<name>/`: model and storage, controller, a module carrying its own policy rows, CSRF exemption, embedded migrations (applied on start) and page template, and a test that boots the slice in-process (`pkg/nucleustest`) and drives its API over HTTP. Mounting it needs no `rbac_policy.csv` or `nucleus.yml` edits, and no migrate step; `--mount` writes the import and the `Mount(<name>.Module())` call into the `nucleus.New()` chain of `main.go` (a composition root without such a chain is left alone and the two lines are printed instead). `--data quark` renders the storage on the Quark ORM over the framework-managed pool. The command ends with `go mod tidy`, which resolves what the slice and its test import (`pkg/nucleustest`, and Quark with its driver module for `--data quark`), so `go test ./...` works as written on any `--db` engine; `--offline` skips it and hands it back. A module name that collides with an import of `main.go` (`log`, `nucleus`) is refused before anything is written. Only `generate resource` creates the `internal/contracts` OpenAPI aggregator. |
